...
def build_pipeline(
    ...
    if include_batch_correction:
        batch_method = config.get('preprocessing', {}).get(
...
def run_ablation_study(
    ...
    for test_config in ablation_tests:
        ...
        feature_method = config.get('features', {}).get(
            'routes', ['filter_l1'])[0]
        model_name = list(config.get('models', {}).keys())[0]

        # Build pipeline
        include_batch = config.get('preprocessing', {}).get(
            'batch_correction') is not None
        pipeline = build_pipeline(
            feature_method, model_name, config, include_batch)
