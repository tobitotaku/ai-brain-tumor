...
**Status:** ⚠️ Training interrupted - laptop crash (resume needed)  
**Branch:** `retake/musab`  
**Date:** 9 November 2025  
**Last Update:** Auto-generated by protocol documentation agent  
...
# Monitor progress
tail -f training_$(ls -t training_*.log | head -1)
```

**Option 3: Smoke Test First**
```bash
# Validate everything works before 3-hour run
python scripts/train_cv.py --config config_smoke_test.yaml

# If successful → proceed with full run
```
...
|-------|----------|--------|
| **Training (6 configs × 5 folds)** | 2.5-3 hours | `nested_cv_results.csv`, model files, figures |
